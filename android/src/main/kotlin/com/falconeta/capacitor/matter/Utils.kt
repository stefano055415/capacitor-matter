...
fun <T> MutableList<T>.mapButReplace(targetItem: T, newItem: T) = map {
  Log.d(TAG, "mapButReplace targetItem [${targetItem}] newItem [${newItem}]")
  if (it == targetItem) {
    Log.d(TAG, "setting newItem for [${it}] to [${newItem}]")
    newItem
  } else {
    Log.d(TAG, "setting newItem")
    it
  }
}
